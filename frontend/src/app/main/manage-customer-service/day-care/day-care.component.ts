 ...
 */
  getVendorList = () => {
    this._authService.request('get', `services/vendors?service_id=3&booking_date=${this.parentForm.controls.date.value}&booking_time=${this.parentForm.controls.drop_off_time.value}:00`) 
      .subscribe(response => {
 ...
 */
  ...
  initRazorPay = () => {
    /* const rzpConfig = {
      ...
      "amount": this.selectedVendorId['service_charge'] * 100,
      "name": "Petcoset",
    ...
    this._authService.request('post', url , data)
      .subscribe(response => {
        ...
        this.spinner.hide();
        Swal.fire({
          text: 'Day Care Added Successfully!',
          icon: 'success',
          confirmButtonText: 'OKay',
          width: 400,
          allowOutsideClick: false
...
isSubscriptionPrivilegeService = () => {
  this.isSubscription = this.subscriptionDetails.find(x => x.ServiceId === 3) ? true : false;
}
