...
@Component({
  ...
  styleUrls: ['./veterinary-doctor.component.css']
})
export class VeterinaryDoctorComponent implements OnInit {
  ...
  changeFirstBlock = () => {
    ...
    if (!data.date || !data.pick_up_time)
   ...
   */
  getVendorList = () => {
    this._authService.request('get', `services/vendors?service_id=9&booking_date=${this.parentForm.controls.date.value}&booking_time=${this.parentForm.controls.pick_up_time.value}:00`)
      .subscribe(response => {
        if (response.res.length !== 0) {
 ...
 */
  initRazorPay = () => {
    ...
    data.booking_time = `${data.pick_up_time}:00`;
    data.booking_end_time = `${data.drop_off_time}:00`;
    ...
    this._authService.request('post', url, data)
      .subscribe(response => {
        ...
        this.spinner.hide();
        Swal.fire({
          text: 'Veterinary Doctor Request Added Successfully!',
          icon: 'success',
          confirmButtonText: 'OKay',
          width: 400,
          allowOutsideClick: false
 ...
 */
  ...
  isSubscriptionPrivilegeService = () => {
    this.isSubscription = this.subscriptionDetails.find(x => x.ServiceId === 6) ? true : false;
  }
  getCityList = () => {
