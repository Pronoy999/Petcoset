...
export class GroomingComponent implements OnInit {
   ...
   */
  getVendorList = () => {
    this._authService.request('get', `services/vendors?service_id=6&booking_date=${this.parentForm.controls.date.value}&booking_time=${this.parentForm.controls.pick_up_time.value}:00`)
      .subscribe(response => {
        if (response.res.length !== 0) {
  ...
  initRazorPay = () => {
    ...
    this._authService.request('post', url, data)
      .subscribe(response => {
        ...
        this.spinner.hide();
        Swal.fire({
          text: 'Grooming Request Added Successfully!',
          icon: 'success',
          confirmButtonText: 'OKay',
          width: 400,
          allowOutsideClick: false
