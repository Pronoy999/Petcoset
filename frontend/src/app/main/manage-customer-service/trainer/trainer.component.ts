...
export class TrainerComponent implements OnInit {
  ...
  */
  ...
  selectedPet = (index) => {
    ...
    this.selectedPetType = this.selectedPetList['pet_type'];
 ...
 */
  getVendorList = () => {
    this._authService.request('get', `services/vendors?service_id=5&booking_date=${this.parentForm.controls.date.value}&booking_time=${this.parentForm.controls.pick_up_time.value}:00`)
      .subscribe(response => {
        if (response.res.length !== 0) {
          ...
          console.log('vendor list', response.res);
        }
      });
  }
  ...
  initRazorPay = () => {
    ...
    this._authService.request('post', url, data)
      .subscribe(response => {
        ...
        this.spinner.hide();
        Swal.fire({
          text: 'Trainer Request Added Successfully!',
          icon: 'success',
          confirmButtonText: 'OKay',
          width: 400,
          allowOutsideClick: false
  ...
  */
  ...
  isSubscriptionPrivilegeService = () => {
    this.isSubscription = this.subscriptionDetails.find(x => x.ServiceId === 5) ? true : false;
  }
