DROP PROCEDURE IF EXISTS sp_insertCustomer;
create procedure sp_insertCustomer(IN firstName varchar(255), IN lastName varchar(255),
                                                          IN emailId varchar(255), IN phoneNumber varchar(13),
                                                          IN customerGender varchar(1), IN address1 varchar(255),
                                                          IN address2 varchar(255), IN cityId int, IN stateId int,
                                                          IN countryId int, IN pin int, IN ownReferralCode varchar(50),
                                                          IN referralCode varchar(50))
BEGIN
    DECLARE customerID INT;
    DECLARE var_emailValidation tinyint;

    SELECT AUTO_INCREMENT
    INTO customerID
    FROM information_schema.TABLES
		WHERE TABLE_NAME = 'tbl_CustomerMaster'
			AND TABLE_SCHEMA = (SELECT DATABASE());

      SET var_emailValidation = (SELECT 1 from tbl_CustomerMaster where email = emailId);
      if(var_emailValidation <> 1)
      Then select -1;
      else
			INSERT INTO tbl_CustomerMaster
            (
				first_name,
                last_name,
                email,
                phone_number,
                gender,
                address_1,
                address_2,
                city,
				state,
                country,
                pincode,
                referral_code,
                used_referral_code,
                created_by
			)
		VALUES
			(
				firstName,
                lastName,
                emailId,
                phoneNumber,
                customerGender,
                address1,
                address2,
                cityId,
                stateId,
                countryId,
				pin,
                ownReferralCode,
				referralCode, 1
			);
		SELECT customerID;
    end if;
end;

